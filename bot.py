...
from aiogram.types import ReplyKeyboardRemove, MenuButtonDefault, FSInputFile
...
def load_media():
    ...
    rows = media_cursor.fetchall()

    # ⚠️ ожидаем структуру таблицы:
    # code, type, title, year, description, country, director,
    # video, poster, genres, seasons, episodes, episode_counter

    for row in rows:
        (
            code, mtype, title, year, description,
            country, director, video, poster,
            genres, seasons, episodes, episode_counter
        ) = row
        ...
        # ✅ genres теперь МАССИВ
        try:
            item["genres"] = json.loads(genres) if genres else []
        except:
            item["genres"] = []

        # сезоны / серии тоже JSON
        try:
            item["seasons"] = json.loads(seasons) if seasons else {}
        except:
            item["seasons"] = {}

        try:
            item["episodes"] = json.loads(episodes) if episodes else []
        except:
            item["episodes"] = []
