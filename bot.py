...
async def open_item(callback: types.CallbackQuery):
    ...
    if not await is_subscribed(user_id):
        if item_type == "movie":
            ...
            # показываем постер + кнопки подписки
            serial = series[code]
            text = (
                f"<b>⭐️ «{serial['title']}», {serial['year']}</b>\n\n"
                f"<i>{serial['description']}</i>\n\n"
                f"<u>Жанр:</u> {' '.join(f'#{g.replace(' ','_')}' for g in serial.get('genres', []))}\n\n"
    ...
    if item_type == "movie":
        ...
        # показываем постер сериала с кнопкой выбора серии
...
async def check_subscription_callback(callback: types.CallbackQuery):
    ...
    await send_episode(callback, code, int(episode_index))

async def check_subscription(user_id):
    chat_id = "@kinonawe4er"  # твой канал
