...
def load_media_data():
    ...
    print("Первые 5 кодов фильмов:", list(movies.keys())[:5])
    if movies:
        code = list(movies.keys())[0]
        print(f"Пример фильма ({code}):", movies[code]["title"])
    
    print("Первые 5 кодов сериалов:", list(series.keys())[:5])
    if series:
        code = list(series.keys())[0]
        print(f"Пример сериала ({code}):", series[code]["title"])
