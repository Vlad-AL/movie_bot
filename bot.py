...
def get_series(code):
    cursor.execute("SELECT * FROM series WHERE code = ?", (code,))
    row = cursor.fetchone()
    if not row:
        return None
...
def get_episodes(code, season=None):
    if season is not None:
        ...
        SELECT episode_number, video
        FROM episodes
        WHERE series_code = ? AND season_number IS NULL
        ORDER BY episode_number
        """, (code,))

    return [{"video": r[1]} for r in cursor_media.fetchall()]
...
def get_all_series():
    cursor_media.execute("SELECT * FROM series")
    rows = cursor_media.fetchall()
...
def seasons_keyboard(code: str):
    cursor_media.execute("""
        ...
        FROM episodes
        WHERE series_code = ? AND season_number IS NOT NULL
        ORDER BY season_number
    """, (code,))

    seasons = [row[0] for row in cursor_media.fetchall()]
    ...
    for season in seasons:
        keyboard.append([
            InlineKeyboardButton(
                text=f"Сезон {season}",
                callback_data=f"season:{code}:{season}"
            )
        ])
...
def search_by_code(query: str):
    ...
    if movie:
        return [("movie", query, movie)]

    serial = get_series(query)
    if serial:
        return [("series", query, serial)]
